...
void SftpOutgoingPacket::setFileNameCodec(const QByteArray &codecName)
{
...
SftpOutgoingPacket &SftpOutgoingPacket::generateRealPath(const QString &path, quint32 requestId)
{
...
{
    m_data.append(AbstractSshPacket::encodeString(encodeFileName(string)));
    return *this;
}
...
QByteArray SftpOutgoingPacket::encodeFileName(const QString &string) const
{
    QTextCodec *codec = QTextCodec::codecForName(m_fileNameCodecName);
    return codec ? codec->fromUnicode(string) : string.toUtf8();
}
