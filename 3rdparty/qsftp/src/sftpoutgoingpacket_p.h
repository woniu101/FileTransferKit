...
public:
    SftpOutgoingPacket();
    void setFileNameCodec(const QByteArray &codecName);
    SftpOutgoingPacket &generateInit(quint32 version);
    SftpOutgoingPacket &generateRealPath(const QString &path, quint32 requestId);
...
private:
    QByteArray encodeFileName(const QString &string) const;
