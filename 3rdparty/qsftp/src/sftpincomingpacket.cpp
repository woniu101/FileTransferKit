...
SftpIncomingPacket::SftpIncomingPacket()
    : m_length(0), m_fileNameCodecName("UTF-8")
{
}

void SftpIncomingPacket::setFileNameCodec(const QByteArray &codecName)
{
...
{
    SftpFile file;
    file.fileName = decodeFileName(SshPacketParser::asString(m_data, &offset));
    file.longName = decodeFileName(SshPacketParser::asString(m_data, &offset));
    file.attributes = asFileAttributes(offset);
    return file;
}

QString SftpIncomingPacket::decodeFileName(const QByteArray &string) const
{
    QTextCodec *codec = QTextCodec::codecForName(m_fileNameCodecName);
    return codec ? codec->toUnicode(string) : QString::fromUtf8(string);
}
