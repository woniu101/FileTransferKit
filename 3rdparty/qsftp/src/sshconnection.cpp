...
//        Q_FUNC_INFO, m_incomingData.count(), m_incomingData.data());
    ...
    // Be permissive for real-world servers such as WeOnlyDo, whose software
    // version contains '-'. The protocol version is validated separately below.
    const QRegExp versionIdpattern(QLatin1String("SSH-([^\\s-]+)-([^\\s]+)(?: .+)?"));
...
void SshConnectionPrivate::handleUserAuthSuccessPacket()
{
    ...
    // Some Windows SFTP servers ignore SSH_MSG_UNIMPLEMENTED for invalid
    // keepalive packets, which makes the connection time out during transfers.
    // Applications can implement their own higher-level timeout/retry policy.
}
