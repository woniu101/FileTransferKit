...
void SftpChannel::setFileNameEncoding(FileNameEncoding encoding)
{
    setFileNameCodecName(encoding == GbkFileNameEncoding
        ? QByteArray("GBK") : QByteArray("UTF-8"));
}

SftpChannel::FileNameEncoding SftpChannel::fileNameEncoding() const
{
    return QString::fromLatin1(d->m_fileNameCodecName).compare(QLatin1String("GBK"),
        Qt::CaseInsensitive) == 0
        ? GbkFileNameEncoding : Utf8FileNameEncoding;
}

void SftpChannel::setFileNameCodecName(const QByteArray &codecName)
{
    const QByteArray normalizedCodecName = QTextCodec::codecForName(codecName)
        ? codecName : QByteArray("UTF-8");
    d->setFileNameCodecName(normalizedCodecName);
}

QByteArray SftpChannel::fileNameCodecName() const
{
    return d->m_fileNameCodecName;
}

QString SftpChannel::initialRemoteDirectory() const
{
    return d->m_initialRemoteDirectory;
}
...
SftpChannelPrivate::SftpChannelPrivate(quint32 channelId,
    ...
    : AbstractSshChannel(channelId, sendFacility),
      m_fileNameCodecName("UTF-8"), m_nextJobId(0), m_realPathJobId(SftpInvalidJob),
      m_sftpState(Inactive), m_sftp(sftp)
{
    m_outgoingPacket.setFileNameCodec(m_fileNameCodecName);
    m_incomingPacket.setFileNameCodec(m_fileNameCodecName);
}
...
void SftpChannelPrivate::setFileNameCodecName(const QByteArray &codecName)
{
    m_fileNameCodecName = codecName;
    m_outgoingPacket.setFileNameCodec(codecName);
    m_incomingPacket.setFileNameCodec(codecName);
}
...
void SftpChannelPrivate::handleServerVersion()
{
    ...
    if (serverVersion != ProtocolVersion) {
        ...
        m_realPathJobId = ++m_nextJobId;
        sendData(m_outgoingPacket.generateRealPath(QLatin1String("."), m_realPathJobId).rawData());
        m_sftpState = RealPathSent;
    }
}
...
void SftpChannelPrivate::handleStatus()
{
    ...
    if (m_sftpState == RealPathSent && response.requestId == m_realPathJobId) {
        m_nextJobId = 0;
...
void SftpChannelPrivate::handleName()
{
    ...
    if (m_sftpState == RealPathSent && response.requestId == m_realPathJobId) {
        if (!response.files.isEmpty())
            m_initialRemoteDirectory = response.files.first().fileName;
        m_nextJobId = 0;
        m_sftpState = Initialized;
        emit initialized();
        return;
