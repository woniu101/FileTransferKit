...
//    qDebug("QFtpPI state: %d [connected()]", state);
	...
	qintptr ptrdiff = commandSocket.socketDescriptor();
	enableKeepalive(ptrdiff);
    ...
    : QObject(parent), d(new QFtpPrivate(this))
{
    ...
    connect(&d->pi, SIGNAL(commandSent(QString)),
            SIGNAL(ftpCommandSent(QString)));
...
{
    emit q_func()->ftpReply(code, text);
