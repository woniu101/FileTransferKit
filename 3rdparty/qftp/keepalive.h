...
const int keepalive = 1;    // 开启keepalive属性,缺省值:0(关闭)
const int keepidle = 5;     // 如果连接在5秒内没有任何数据来往则进行探测,缺省值:7200(s)
const int keepinterval = 3; // 探测时发包的时间间隔为3秒,缺省值:75(s)
const int keepcount = 3;    // 探测重试的次数,全部超时则认定连接失效,缺省值:9(次)
